...
//        strOut.append(Utils.PadRight("", iLevel * 4 + 1) + "msg_text_id=" + menu.msg_text_id + " [" + menu.msg_text + "] " + "\r\n" );
//        if (menu.menu_item_id >= 0)
//            strOut.append(Utils.PadRight("", iLevel * 4 + 1) + "menu_item_id=" + menu.menu_item_id + "\r\n" );
//
...
//    }
            ...
            while (i < tagList.length)
            {
                ...
                if (tag.bConstructed) {
                    ...
                    if (tag.iTag == 0xFF8022) {                         // menu directory
                        ...
                        if (menu == null) {
                            menu = new MenuElement(true);
                            int iRet = GetTLVMenu(baValue, menu );
                            if (iRet != 0)  return iRet;
                            menu.getSubmenu().add(menu );

                        } else {
                            MenuElement menuLocal = new MenuElement(true);
                            menuLocal.setPreviousMenuElement(menu);
                            int iRet = GetTLVMenu(baValue, menuLocal);
                            if (iRet != 0)  return iRet;
                            menu.getSubmenu().add(menuLocal);
                        }


                    } else if (tag.iTag == 0xFF8023) {                  // menu item
                        MenuElement menuLocal = new MenuElement(false);
                        int iRet = GetTLVMenu(baValue, menuLocal );
                        if (iRet != 0)  return iRet;
                        menu.getSubmenu().add(menuLocal);
                    }
                    ...
                    if (menu != null) {
                        if          (tag.iTag == 0xDF01)        menu.setMenu_item_id(lValue);
                        else if     (tag.iTag == 0xDF8030)      menu.setPw_level(lValue);
                        else if     (tag.iTag == 0xDF8032)      menu.setHidden(lValue);
                        else if     (tag.iTag == 0xDF8033)      menu.setNumber(lValue);
                        else if     (tag.iTag == 0xDF8046)      menu.setItem_property(lValue);
                        else if     (tag.iTag == 0xDF8048)      menu.setPre_param_id(lValue);
                        else if     (tag.iTag == 0xDF8049) {
                            menu.setMsg_text_id(lValue);
                            menu.setMsg_text(getMsgText((int)lValue));
                        }
                    }
                }
            }
    ...
    public static void InterpretTLV(byte[] tagList, Integer iLevel, StringBuilder strOut) {
        int i = 0;
        byte j = 0;
        int tag = 0;
        int tagLength = 0;
        ...
        try {
            ...
            while (i < tagList.length)
            {
                ...
                if (bConstructed || tag == 0x0000DF53 || tag == 0x0000DF7C || tag == 0x0000DF7B) {
                    strOut.append(Utils.PadRight("", iLevel * 4 + 1) + FormatTag(tag) + FormatTagName(tag) + FormatLength(tagLength) + "\r\n" );

                    InterpretTLV(baValue, iLevel + 1, strOut);
                } else {
                    strOut.append(Utils.PadRight("", iLevel * 4 + 1) + FormatTag(tag) + FormatTagName(tag) + FormatLength(tagLength) + "=" +
                            (IsTagDisplay(tag) ? Utils.FilterString(new String(baValue, "Cp1252")) + " - " : ""   ) +  Utils.Byte2Hex(baValue) + "\r\n");
                }
            }
        } catch (Exception e) {
            strOut.append(e.toString() + "\r\n");
    ...
    static String FormatTagName(int iTag) {
        return Utils.PadRight("'" + GetTagName(iTag) + "'",35);
    }

    static String FormatTag(int iTag) {
        return Utils.PadRight(Integer.toHexString(iTag).toUpperCase(), 8);
    }

    static String FormatLength(Integer iLen) {
        return "(" + Utils.PadLeft(iLen.toString(), 2) + ")";
    }

    static byte[] GetTagValue(byte[] tagList, int getTag) {
        int i = 0;
        byte j = 0;
        int tag = 0;
        int tagLength = 0;
    ...
    static byte[] CreateTag(int iTag, byte[] baValue) {
        ...
        try {
            ...
            System.out.println("CreateTag: " + e.toString());
            return null;
        }
    }

    static byte[] AppendTag(byte[] baTLV, int iTag, byte[] baValue) {
        ...
        try {
            ...
            System.out.println("AppendTag: " + e.toString());
            return null;
        }

    }

    static byte[] MakeLength(int iLen) {
        if (iLen < 0x80) {
    ...
    static public String GetTagName2(int iTag)
    {
        switch (iTag)
        {
    ...
    static public String GetTagName(int iTag)
    {
        switch (iTag)
        {
    ...
    static boolean IsTagDisplay(int iTag) {
        switch (iTag) {
